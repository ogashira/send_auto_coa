...
class ExportPaintList(object):
    ...
    def __init__(self, delivery_date: str) -> None:
        self.__delivery_date: str = delivery_date
        ...
        ws = wb['輸出塗料連絡表']
        self.__all_data = [[cell.value for cell in row] for row in ws]
        # 先頭２行は削除する
        self.__all_data = self.__all_data[2:]


    def get_export_paints(self)-> List[List[str]]:

        export_paints: List[List[str]] = []
        for row in self.__all_data: #self.all_data == 輸出塗料連絡表
            row_list: List[str] = []
            if isinstance(row[2], datetime.datetime):
                str_deli_day = row[2].strftime('%Y%m%d')
                if str_deli_day == self.__delivery_date:
